...
cyan = (83, 165, 184)
dark_pink = (76, 32, 84)
foreground_color = (130, 130, 130)   # dark brownish-green
vegetation_color = (150, 150, 150)  # lighter green for small plants
...
def addNoise(color, scale=1):
    return weightedAvg([random.randint(150,250) for ii in range(3)], color, random.random()*scale, random.random())
    ...
    def generate(self):
        ...
        for leaf in self.leaves2:
            for i in range(int(self.canopy_info.density // 2 * self.growth**0.5)):
                ...
                self.leaves2_g.append((pos, 10, random.random()*360, weightedAvg(fade, addNoise(pink, 1), self.depth*0.333, 1-self.depth*0.333)))
        ...
        for leaf in self.leaves3:
            for i in range(int(self.canopy_info.density * self.growth**0.5)):
                ...
                angle = random.random()*2*math.pi
                pos = addVector(leaf, r, angle, stretch=self.canopy_info.stretch)
                self.leaves3_g.append((pos, 10, random.random()*360, weightedAvg(fade, addNoise(pink, 0.5), self.depth*0.333, 1-self.depth*0.333)))
...
def generateRandomTree():
    tree_info = random.choice([Kapok_info, BigLeafMahogany_info, Shrub_info])
    generateTree(
        name=tree_info.name,
        base_info=AttrDict({
...
POMODORO_WORK = 10 * 60   # 25 minutes
