...
folder = "playlist"
files = [f[:-4] for f in os.listdir(folder) if f.endswith(".mp4")]
files2 = [f[:-4] for f in os.listdir(folder) if f.endswith(".mp3")]

for f in files:
    if f in files2: continue
    audio = AudioSegment.from_file(os.path.join(folder, f), format="mp4")
    audio.export(f"{os.path.join(folder, f)}.mp3", format="mp3")
...
class Tree():
    def __init__(self, name, base, height, leaves, canopy_info, colors):
        if len(trees) > 20: self.depth = random.choice([0,1,1,2,2,2,2,3,3])
        else: self.depth = depths[len(trees)]

        if name == "Maple": self.depth = 0
    ...
    def generate(self):
        ...
        for leaf in self.leaves:
            ...
            for i in range(int(self.canopy_info.density * self.growth**0.5)):
                ...
                w = (r * math.sin(angle) / self.canopy_info.spread +1)/2 * 0.3 + (math.sin(angle)+1)/2 * 0.7
                color = weightedAvg(fade, weightedAvg(
                    self.colors.leaf1,
                    self.colors.leaf2,
                    w,
                    1-w,
                ), self.depth_display*0.3, 1-self.depth_display*0.3)
        ...
        for leaf in self.leaves2:
            for i in range(int(self.canopy_info.density // 2 * self.growth**0.5)):
                ...
                self.leaves2_g.append((pos, 10, random.random()*360, weightedAvg(fade, self.colors.leaf1, self.depth_display*0.3, 1-self.depth_display*0.3)))
        ...
        for leaf in self.leaves3:
            for i in range(int(self.canopy_info.density * self.growth**0.5)):
                ...
                angle = random.random()*2*math.pi
                pos = addVector(leaf, r, angle, stretch=self.canopy_info.stretch)
                self.leaves3_g.append((pos, 10, random.random()*360, weightedAvg(fade, self.colors.leaf2, self.depth_display*0.3, 1-self.depth_display*0.3)))
    ...
    def display(self):
        ...
        while parseme:
            ...
            node.display(weightedAvg(fade, weightedAvg(
                ...
                w,
                (1-w)
            ), self.depth_display*0.3, 1-self.depth_display*0.3), surface=self.surface)
            parseme += node.children
...
PurpleHeart_info = AttrDict({
        ...
        "trunk_info": AttrDict({
            ...
            "max_height": 600, # max height of trunk
        ...
        "branch_info": AttrDict({
            ...
            "branch_probs": [0.5, 0.3, 0.2], # probability of branch per branch segment
...
BigLeafMahogany_info = AttrDict({
        ...
        "branch_info": AttrDict({
            ...
            "branchoff_angle_randomness": 0, # +- randomness to branchoff angle
            "branchoff_prob": 0.4, # probability of branch per trunk segment
            "branch_probs": [0.45, 0.35, 0.2], # probability of branch per branch segment
            "branch_preference": 0.4, # up/down
            "branch_angle": 30,
            "min_seg": 6, # minimum branch segments
            "max_seg": 8, # maximum branch segments
...
Cacao_info = AttrDict({
        ...
        "trunk_info": AttrDict({
            "min_height": 150, # min height of trunk
            "max_height": 250, # max height of trunk
            "min_width": 3, # min width of trunk
            "angle_randomness": (0,3), # +- randomness to trunk angle
            "taper": 0.05, # taper constant
            "taper_rate": 1, # taper rate
            "seg_min_height": 1, # min height multiplier for trunk segment
            "seg_max_height": 1, # max height multiplier for trunk segment
        ...
        "branch_info": AttrDict({
            "branch_max_height": 10,
            "branchoff_min_height": 0.0, # minimum height where branches can start
            "branchoff_angle": 30, # angle with respect to parent trunk/branch branches off
            "branchoff_angle_randomness": 0, # +- randomness to branchoff angle
            "branchoff_prob": 0.9, # probability of branch per trunk segment
            "branch_probs": [0.4, 0.4, 0.2], # probability of branch per branch segment
            "branch_preference": 0.25, # up/down
            "branch_angle": 30,
            "min_seg": 1, # minimum branch segments
            "max_seg": 3, # maximum branch segments
            "width": 1, # width of branch
            "seg_height": 20, # height of segment
            "seg_min_height": 0.5, # min height multiplier for branch segment
            "seg_max_height": 1, # max height multiplier for branch segment
...
Maple_info = AttrDict({
        ...
        "trunk_info": AttrDict({
            "min_height": 300, # min height of trunk
            "max_height": 400, # max height of trunk
            "min_width": 4, # min width of trunk
            "angle_randomness": (0,0), # +- randomness to trunk angle
            "taper": 0.05, # taper constant
            "taper_rate": 1, # taper rate
            "seg_min_height": 1, # min height multiplier for trunk segment
        ...
        "branch_info": AttrDict({
            "branch_max_height": 20,
            "branchoff_min_height": 0.35, # minimum height where branches can start
            "branchoff_angle": 30, # angle with respect to parent trunk/branch branches off
            "branchoff_angle_randomness": 0, # +- randomness to branchoff angle
            "branchoff_prob": 0.4, # probability of branch per trunk segment
            "branch_probs": [0.45, 0.35, 0.2], # probability of branch per branch segment
            "branch_preference": 0.8, # up/down
            "branch_angle": 30,
            "min_seg": 6, # minimum branch segments
            "max_seg": 8, # maximum branch segments
            "width": 2.5, # width of branch
...
def generateRandomTree():
    ...
    if len(trees) == 0:
        tree_info = random.choice([Kapok_info, PurpleHeart_info, BigLeafMahogany_info, Cacao_info])
        generateTree(
            name=tree_info.name,
            base_info=AttrDict({
                "base_pos": ((random.random()-0.5)*1000 + screen_size[0]/2,1020), # position of base of tree
        ...
        tree_info = random.choice([Maple_info])
        ...
        # tree_info = random.choice([Kapok_info, PurpleHeart_info, BigLeafMahogany_info, Shrub_info, Cacao_info, Maple_info])
        tree_info = Maple_info
...
class RainDrop:
    def __init__(self, screen_w, screen_h):
        ...
        self.x = random.randint(0, screen_w*1.5) - 500
    ...
    def update(self, screen_w, screen_h):
        self.x += self.dx
        self.y += self.dy
        if self.y > 0.78*screen_h or self.x > screen_w:
            self.length -= 6
            if self.length <= 0:
    ...
    def draw(self, surface):
        ...
        pygame.draw.line(surface, (180, 180, 200, 80 - 50*self.depth), (self.x, self.y), (end_x, end_y), 4)
...
def play_random_song():
    ...
    while track == current_track:
        track = os.path.join(folder, random.choice(files))
